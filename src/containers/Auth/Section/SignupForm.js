...
class SignupForm extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({
            ...
            signupData: {
                ...
                role: (this.props.roles[0] && this.props.roles[0].key) || '',
            },
        });
    }

    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.genders !== prevState.genders) {
    ...
    handleInputOnChange = async (event) => {
        ...
        const { email, password, phoneNumber, firstName, lastName, address, avatar } =
            this.state.signupData;
        ...
        if (
            ...
            document.querySelector(`#signup_form .${style.form_button}`).style =
                'pointer-events: all; opacity: 1;';

            document
                .getElementById('signup_form')
                .addEventListener('submit', this.handleSubmitForm);
        }
    ...
    handleSubmitForm = async (event) => {
        ...
        try {
            const res = await userService.handleSignup(this.state.signupData);
    ...
    render() {
        const { email, password, phoneNumber, gender, firstName, lastName, role, address, avatar } =
            this.state.signupData;
        ...
        return (
            <form id='signup_form' onSubmit={(event) => event.preventDefault()}>
                ...
                <button className={`col-12 ${style.form_button}`} form='signup_form' type='submit'>
...
const SelectInput = ({ col, id, name, onChangeHandler, defaultValue, options, language }) => {
    return (
        <div className={`form-group col-${col} ${style['form-group']}`}>
            <label htmlFor={id}>
                <FormattedMessage id='signup.role' />
            </label>
            <select
                id={id}
